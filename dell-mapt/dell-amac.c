...
static int __init acpi_amac_init(void)
{
	...
	if (ACPI_FAILURE(status)) {
		pr_err("Failed to evaluate ACPI object %s: %s\n", mac_obj_name, acpi_format_exception(status));
	...
	if (obj->type != mac_obj_type || obj->string.length != mac_strlen) {
		pr_err("Invalid buffer for pass-thru MAC addr: (%d, %d)\n",
			   obj->type,
			   obj->string.length);
		goto amacout;
	}
	...
	if (strncmp(obj->string.pointer, "_AUXMAC_#", 9) != 0 ||
		...
		pr_err("Invalid header when reading pass-thru MAC addr\n");
		goto amacout;
	}

	ret = hex2bin(buf, obj->string.pointer + 9, 6);
	if (!(ret == 0 && is_valid_ether_addr(buf))) {
		pr_err("Invalid MAC for pass-thru MAC addr: %d, %pM\n",
			   ret, buf);
		ret = -EINVAL;
		goto amacout;
	}
	...
	*/
	pr_info("Using pass-thru MAC addr %pM\n", buf);
